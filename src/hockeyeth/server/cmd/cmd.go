...
func StartCPUProf(cpuProf bool, prior *os.File) *os.File {
	if prior != nil {
		...
		prior.Close()
		os.Rename(filepath.Join(os.TempDir(), "hockeyeth-cpu.prof.part"),
			filepath.Join(os.TempDir(), "hockeyeth-cpu.prof"))
	}
	if cpuProf {
		profName := filepath.Join(os.TempDir(), "hockeyeth-cpu.prof.part")
...
func WriteMemProf(memProf bool) {
	if memProf {
		tmpName := filepath.Join(os.TempDir(), fmt.Sprintf("hockeyeth-mem.prof.%d", time.Now().Unix()))
		profName := filepath.Join(os.TempDir(), "hockeyeth-mem.prof")
