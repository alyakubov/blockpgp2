...
import (
	...
	hkpstorage "hockeyeth/hkp/storage"
	log "hockeyeth/logrus"
	...
	for {
		...
		for _, event := range announceEvents {
			...
			if isFoundAndEqual == -1 {
				...
				_, _, err = stor.Insert([]*openpgp.PrimaryKey{&primKey})
				if err != nil {
			...
			if needUpsert, err := st.insertKey(key); err != nil {
				...
				if err == errTargetMissing {
					...
					case hkpstorage.KeyReplaced:
						// FIXME: Listener in hockeyeth-load not really prepared for
...
func parseArmored(armouredContent []byte) (primKey openpgp.PrimaryKey, err error) {
	pgpCerts, err := openpgp.ReadArmorKeys(bytes.NewReader(armouredContent))
	if err != nil {
		return primKey, fmt.Errorf("Cannot parse armored cert: %w", err)
	}

	if len(pgpCerts) > 1 {
		return primKey, fmt.Errorf("Received more than 1 PGP cert from armorParse")
	}
	if len(pgpCerts) == 0 {
		return primKey, fmt.Errorf("Received 0 PGP cert from armorParse")
	}

	return *pgpCerts[0], nil
	/*
		...
		for pgpCert := range chPgpCert {
			// arrPgpCert = append(arrPgpCert, *pgpCert.PrimaryKey) // *pgpCert.PrimaryKey
			// Assuming pgpCert is of type *openpgp.Entity
			if cert, ok := pgpCert.(*openpgp.Entity); ok {
				arrPgpCert = append(arrPgpCert, *cert.PrimaryKey)
			} else {
				return primKey, fmt.Errorf("unexpected type in channel: %T", pgpCert)
			}
		}
		if len(arrPgpCert) > 1 {
