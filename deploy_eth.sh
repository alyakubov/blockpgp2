...
gethWDir=/home/alex/win/ether-data
passwrd="ira"
...
EOF
    ...
    if [ "$isLib" = "$binfile" ]; then
        echo "Saving to contract address file $1:$address"
        echo "$1:$address" >> "$outputPath/$addrFile"
    else
        echo "Saving to library address file $1:$address"
...
init_sc() {
    ...
    pgp.Init.sendTransaction( "$cOrchestrAddr",
        {
            from:web3.eth.accounts[0], gas: 8000000
        }, function(e, contract){
            if(!e) {
                console.log("SUCCESS");
            } else {
...
main() {
    ...
    docker run -v $(pwd)/scontract:/sources ethereum/solc:0.4.22 -o /sources/binout --abi --bin --overwrite /sources/corchestr.sol
    ...
    if [ -f "$outputPath/$addrLibFile" ] ; then
        ...
        docker run -v $(pwd)/scontract:/sources ethereum/solc:0.4.22 -o /sources/binout --bin --overwrite /sources/corchestr.sol --libraries /sources/binout/"$addrLibFile"
        printf "LU_PGP_orchestr OK\n"
        docker run -v $(pwd)/scontract:/sources ethereum/solc:0.4.22 -o /sources/binout --bin --overwrite /sources/ciden.sol --libraries /sources/binout/"$addrLibFile"
        printf "LU_PGP_iden OK\n"
    else
